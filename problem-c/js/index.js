...
function extractOpponent(balls) {
  if(balls.home == "UW") {
    return balls.opponent;
  }
...
function huskiesLost(balls){
  let ret = balls.home_score - balls.opponent_score;
  if(balls.home == "UW") {
    return ret < 0;
  }
...
loser.forEach(
  function(balls){
    console.log(
        balls.opponent+" at "+balls.home+", "+balls.opponent_score+" to "+balls.home_score
      );
  }
);
...
console.log(
  huskyGames2016.filter(
    function(balls){
      return balls.fumbles > 0;
    }
).length);
...
function mostYardsPassing(balls1, balls2) {
  if(Object.keys(balls1).includes("passing_yards")){
    if(balls1.passing_yards > balls2.passing_yards) {
      return balls1;
    }
  }
...
console.log(huskyGames2016.reduce(mostYardsPassing));
...
function makeCombinedFilter(fx1, fx2) {
  return function f (balls) {
    return fx1(balls) && fx2(balls);
  };
}
...
let fumbledAndLostFilter = makeCombinedFilter(huskiesLost, function(balls){return balls.fumbles > 0});
